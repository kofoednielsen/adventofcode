...
fn run(stack: &mut [i32]) {
    ...
    loop {
        ...
        match opcode {
            ...
            JIT => {
                if getvalue(stack, ip+1, par1) != 0 {
                    ip = getvalue(stack, ip+2, par2) as usize;
                } else {
                    ip += 3;
                }
            }
            ...
            LEQ => {
                let result = getvalue(stack, ip+1, par1) < getvalue(stack, ip+2, par2);
                stack[stack[ip+3] as usize] = result as i32;
                ip += 4;
            },
            EQ => {
                let result = getvalue(stack, ip+1, par1) == getvalue(stack, ip+2, par2);

...
fn main() -> io::Result<()> {
    ...
    // Make the lists into hash sets
    let a_set: HashSet<(i32,i32)> = a_points.into_iter().collect();
    let b_set: HashSet<(i32,i32)> = b_points.into_iter().collect();

    // Find intersections between the two lines
    let common = a_set.intersection(&b_set);

    // Map the intersections, into their distance to 0,0 
    // with manhatten distance, this is the same as adding the absolute values
    // of the coordinates
    let mut dists = common.map(|p| p.0.abs() + p.1.abs()).collect::<Vec<i32>>();
